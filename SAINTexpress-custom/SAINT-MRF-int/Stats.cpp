...
Model_data statModel(const vector<vector<size_t> >& p2p_mapping, const vector<string>& ubait, const Quantmat& test_mat_DATA, const Quantmat& ctrl_mat_DATA, const vector<size_t>& ip_idx_to_bait_no, const size_t nprey, const size_t nbait, const deque<PreyClass> prey_class_vec) {
	...
	// Calculate the default value of t as normally would be used by SAINT.
	const Quant_t default_t = accumulate(ctrls_with_NA.cbegin(),ctrls_with_NA.cend(),0.0)/ctrls_with_NA.size();

	// Add a print statement for the default value of t.
	cout << "default_t " << default_t << endl;

	// This vector needs to be indexed the same way so that it can be used in other functions that use t.
	vector<Quant_t> t;
	// use the imputed mu for each prey as the value of t
	for(unsigned i=0; i<test_mat_DATA.n_rows;++i){
		string prey_id = prey_class_vec[i].get_preyId();
		Quant_t imputedMu = prey_class_vec[i].get_imputedMu();

		// Add a print statement to show the prey_id and the imputed mu for that prey.
		// cout << "prey_id " << prey_id << " imputedMu " << prey_class_vec[i].get_imputedMu() << endl;
		
		// Check if imputedMu is NaN or zero
		if (std::isnan(imputedMu) || imputedMu == 0.0 || std::isinf(imputedMu)) {
			// Add a print statement if the imputed mu is NaN or zero.
			cout << "prey_id " << prey_id << " imputedMu is NaN, inf, or zero." << endl;
			t.push_back(default_t);
		} else {
			t.push_back(imputedMu);
		}

		// // Add a print statement with the prey_id and the imputed mu for that prey and the values from test_mat_DATA.
		// cout << "prey_id " << prey_id << " imputedMu " << prey_class_vec[i].get_imputedMu() << endl;

		// // Add a print statement that prints the prey id of the ith row of test_mat_DATA. This is to check to see if prey_id is the same as the prey_id in the prey_class_vec.
		// cout << "prey_id " << prey_id << " test_mat_DATA(i,0) " << test_mat_DATA(i,0) << endl;


		// auto it = find(ubait.begin(), ubait.end(), prey_id); // it is an iterator
		// if(it != ubait.end()){ //
		// 	// find the index of the prey_id in the ubait vector
		// 	int idx = distance(ubait.begin(), it); //
		// 	// use the imputed mu for the prey as the value of t
			
		// }
		// else{
		// 	// if the prey_id is not in the ubait vector, use the average of the control values as the value of t

		// 	// Add a print statement to show if the prey_id is not in the ubait vector.
		// 	cout << "prey_id " << prey_id << " not found in ubait vector." << endl;

		// 	t.push_back(accumulate(ctrls_with_NA.cbegin(),ctrls_with_NA.cend(),0.0)/ctrls_with_NA.size());
		// }
	}
	...
	for(size_t idx = 0; idx < ctrl_mat_DATA.n_rows; idx++){
		...
		auto tmp = mean_sd_estimate(ctrl, t[idx], sd_NA, lse);
		eta[idx] = tmp.first;
...
double Model_data::llikelihood() const {
	...
	for(size_t i=0; i < nprey; ++i) {
		for(size_t j=0; j < nbait; ++j) {
			...
			double prod = 1;
			for(unsigned rep = 0; rep < m; rep++){
				//double y_rep = saint::isnan(y[rep]) ? t : static_cast<double>(y[rep]);//TODO  // This line uses t as the imputed value for missing values.
				// Need to change that line to use the indexed t vector.
				double y_rep = saint::isnan(y[rep]) ? static_cast<double>(t[i]) : static_cast<double>(y[rep]);// t is a vector of imputed values for each prey.
...
void Model_data::icm_Z() {
	...
	for(size_t i=0; i < nprey; ++i)
		for(size_t j=0; j < nbait; ++j) {
			...
			for(unsigned rep = 0; rep < m; rep++) {
				double y_rep = saint::isnan(y[rep]) ? static_cast<double>(t[i]) : static_cast<double>(y[rep]);//TODO // This line uses t as the imputed value for missing values.
				// Quant_t y_rep = saint::isnan(y[rep]) ? t : y[rep];
...
Fastmat<vector<boost::array<double, 2> > > Model_data::precalculate_densities() const {
	...
	for(size_t i=0; i < nprey; ++i)
		for(size_t j=0; j < nbait; ++j) {
			...
			for(unsigned rep = 0; rep < m; rep++){
				double y_rep = saint::isnan(y[rep]) ? static_cast<double>(t[i]) : static_cast<double>(y[rep]);//TODO
				// Quant_t y_rep = saint::isnan(y[rep]) ? t : y[rep];
...
boost::array<Fastmat<double>, 3> Model_data::calculateScore() const {
	Fastmat<double> average_score(nprey, nbait); // This initializes an empty matrix
	Fastmat<double> min_log_odds_score(nprey, nbait);
	Fastmat<double> maximum_score(nprey, nbait);
	for(size_t i=0; i < nprey; ++i) 
		for(size_t j=0; j < nbait; ++j) { // for each prey and bait
			// const auto& k = Z(i,j);
			const auto& y = test_mat1(i,j); // y is the vector of replicates for prey i and bait j
			const auto m = n_rep_vec[j]; // m is the number of replicates for prey i and bait j
			double gsum = 0; // gsum is the covariate for the gamma term
			// {change to p2p
			// BOOST_FOREACH(const auto l , b2b_mapping[j])
			// 	gsum += Z(i, l).mean();
			// double MRFtrue = exp(beta1 + apply_MRF[i] * gamma * gsum);
			// }
			BOOST_FOREACH(const auto l , p2p_mapping[i]) // p2p_mapping[i] is the vector of baits that prey i is detected in. l is the index of the bait
				gsum += Z(l, j).mean(); // Z(l, j) is the vector of replicates for prey l and bait j
			double MRFtrue = exp(beta1 + gamma * gsum);
			double MRFfalse = exp(beta0);
			// mean scoring
			average_score(i, j) = 0;
